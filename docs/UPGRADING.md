...
From **3.1.4** — No application upgrade steps (dev/demo frontend Dependabot bumps only).
...
Review Flex recipe `security_nowo_pdf_signable.yaml` after update.
...
**Patch release:** No breaking changes and no new bundle configuration keys. Contributor governance (Code of Conduct), Git hygiene tooling for maintainers (REQ-GIT-001), and additional unit tests.
...
- After cloning, run `make setup-hooks` once so `.githooks/commit-msg` strips Cursor co-author trailers (REQ-GIT-001). See [GITHUB_CI](GITHUB_CI.md) and [CONTRIBUTING](CONTRIBUTING.md).
...
**Patch release:** No breaking changes and no new configuration keys. Documentation clarifies Symfony **8.1+** needs PHP **8.4.1+**; CI fix only for maintainers.
...
If you run Symfony **8.1+**, ensure the host application uses PHP **8.4.1** or later (Symfony **8.0** requires PHP **8.4+**).

See [CHANGELOG.md](CHANGELOG.md) for the full list of changes.

---

### Upgrading to 3.0.1 (2026-07-01)

**Release date:** 2026-07-01

**Patch release:** No breaking changes and no new configuration keys. Bug fixes for Symfony Validator 7.4+/8.x (`Choice` constraints) and CI/documentation clarifications.

#### Upgrade steps (from 3.0.0)

1. Run `composer update nowo-tech/pdf-signable-bundle`.
2. Clear cache: `php bin/console cache:clear`.

No YAML or runtime config changes are required.
...
**Patch release:** No breaking changes and no new bundle configuration keys. Maintenance release: Makefile `update-deps` fix, CI matrix (Symfony 7.4 / 8.1), refreshed demo lock files, and contributor documentation.
...
**Patch release:** No breaking changes. Improves development tooling: PHP-CS-Fixer rules (PSR-12/Symfony), Docker (PHP 8.2 Alpine, dev/test deps), demo Makefiles (restart, build, test commands), demos served over HTTP by default, and READMEs for Symfony 7 and 8 demos. CI workflow simplified.
