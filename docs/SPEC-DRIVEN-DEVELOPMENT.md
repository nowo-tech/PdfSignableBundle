...
2. **Product behavior** — what **PdfSignableBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** legal integrator, **I want** drag-and-drop signature boxes on PDFs **so that** signers know exactly where to sign. |
| US-02 | **As an** integrator, **I want** PDF.js-backed form widgets **so that** admins preview documents in the browser. |
| US-03 | **As an** integrator, **I want** validated coordinates and signing hooks **so that** downstream signers receive consistent metadata. |
| US-04 | **As a** maintainer, **I want** PHPUnit on validators **so that** invalid placements are rejected before save. |
...
**Goal:** Define signature box coordinates on PDFs in Symfony: form type with PDF.js viewer, drag-and-drop placement, validation, and signing hooks.
...
| --- | --- | --- |
| REQ-MAKE-008 | Root `Makefile`, `demo/Makefile`, `demo/symfony7/Makefile`, `demo/symfony8/Makefile` | Standard **`update-deps`** (and demo **`update-deps-all`**) targets: refresh Composer dependencies in the bundle and each Symfony demo via shared `bundles/.scripts/` Makefiles. Requires `COMPOSE` and `SERVICE_PHP` before include. |
