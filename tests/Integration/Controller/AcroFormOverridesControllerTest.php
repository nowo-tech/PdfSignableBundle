...
final class AcroFormOverridesControllerTest extends TestCase
{
    ...
    public function testApplyEventSetsModifiedPdfReturns200(): void
    {
        ...
        $dispatcher->method('dispatch')->willReturnCallback(static function (object $event) use ($modifiedPdf): object {
    ...
    public function testLoadOverridesWithExtractorScriptNotFileSetsExtractorError(): void
    {
        $storage    = $this->createMock(AcroFormOverridesStorageInterface::class);
        ...
        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        $data = json_decode($response->getContent(), true, 512, JSON_THROW_ON_ERROR);
        self::assertArrayHasKey('fields_extractor_error', $data);
    }

    public function testLoadOverridesWithExtractorScriptSuccessMergesExtractedFields(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_load_extract_ok_' . getmypid() . '.py';
        file_put_contents($script, "import json\nprint(json.dumps([{\"id\":\"f1\",\"rect\":[0,0,100,20]},{\"id\":\"f2\",\"rect\":[0,30,80,50]}]))\n");
        ...
        try {
            ...
            self::assertCount(2, $data['fields']);
            self::assertArrayHasKey('f2', $data['overrides']);
            self::assertSame('x', $data['overrides']['f1']['defaultValue']);
        } finally {
            @unlink($script);
        }
    }

    public function testApplySkipsNonArrayPatchItems(): void
    {
        $editor = $this->createMock(PdfAcroFormEditorInterface::class);
        $editor->expects(self::once())->method('applyPatches')
            ->with(self::stringContains('%PDF'), self::callback(static fn (array $patches): bool => count($patches) === 1))
            ->willReturn('%PDF-1.4 edited');
        $controller = $this->createController(allowPdfModify: true, editor: $editor);
        $request    = Request::create('/pdf-signable/acroform/apply', 'POST', [], [], [], [
            'CONTENT_TYPE' => 'application/json',
    ...
    public function testProcessWithEmptyDocumentKeyTreatedAsNull(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_process_empty_doc_key_' . getmypid() . '.py';
        file_put_contents($script, "import argparse\np=argparse.ArgumentParser();p.add_argument('--input');p.add_argument('--output');p.add_argument('--document-key', required=False);a=p.parse_args();open(a.output,'wb').write(open(a.input,'rb').read())\n");
        try {
            $controller = $this->createController(processScript: $script);
            $request    = Request::create('/pdf-signable/acroform/process', 'POST', [], [], [], [
                'CONTENT_TYPE' => 'application/json',
    ...
    public function testProcessPassesDocumentKeyArgumentToScript(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_process_doc_key_arg_' . getmypid() . '.py';
        file_put_contents($script, "import argparse,sys\np=argparse.ArgumentParser();p.add_argument('--input');p.add_argument('--output');p.add_argument('--document-key', required=False);a=p.parse_args();\nif a.document_key!='doc-123': sys.exit(2)\nopen(a.output,'wb').write(open(a.input,'rb').read())\n");
        try {
            $controller = $this->createController(processScript: $script);
            $request    = Request::create('/pdf-signable/acroform/process', 'POST', [], [], [], [
                'CONTENT_TYPE' => 'application/json',
    ...
    public function testLoadOverridesWithExtractorScriptFailureSetsExtractorErrorMessage(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_load_extract_fail_' . getmypid() . '.py';
        ...
        try {
            ...
            self::assertSame(Response::HTTP_OK, $response->getStatusCode());
            $data = json_decode($response->getContent(), true, 512, JSON_THROW_ON_ERROR);
            self::assertStringContainsString('could not run', $data['fields_extractor_error']);
        } finally {
            @unlink($script);
        }
    }

    public function testLoadOverridesWithPdfUrlInvalidSkipsExtraction(): void
    {
        $storage    = $this->createMock(AcroFormOverridesStorageInterface::class);
        $controller = $this->createController(storage: $storage, fieldsExtractorScript: '/nonexistent/extractor.py');
        $request    = Request::create('/pdf-signable/acroform/overrides/load', 'POST', [], [], [], [
        ...
        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        $data = json_decode($response->getContent(), true, 512, JSON_THROW_ON_ERROR);
        self::assertArrayNotHasKey('fields_extractor_error', $data);
        self::assertArrayNotHasKey('fields', $data);
    }

    public function testLoadOverridesWithPdfUrlNotAllowedSkipsExtraction(): void
    {
        $storage    = $this->createMock(AcroFormOverridesStorageInterface::class);
        $controller = $this->createController(
            storage: $storage,
    ...
    public function testLoadOverridesWithPdfUrlBlockedBySsrfSkipsExtraction(): void
    {
        $storage    = $this->createMock(AcroFormOverridesStorageInterface::class);
        $controller = $this->createController(storage: $storage, fieldsExtractorScript: '/nonexistent/extractor.py');
        $request    = Request::create('/pdf-signable/acroform/overrides/load', 'POST', [], [], [], [
    ...
    public function testLoadOverridesWithPdfUrlHttpExceptionSkipsExtraction(): void
    {
        $httpClient = $this->createMock(HttpClientInterface::class);
        $httpClient->expects(self::once())->method('request')->willThrowException(new RuntimeException('network'));
    ...
    public function testApplyDebugLogsValidationResult(): void
    {
        $dispatcher = $this->createMock(EventDispatcherInterface::class);
        $dispatcher->method('dispatch')->willReturnCallback(static function (object $event): object {
            if ($event instanceof AcroFormApplyRequestEvent) {
                $event->setValidationResult(['ok' => true]);
            }
        ...
        $logger             = $this->createMock(LoggerInterface::class);
        $logger->method('info')->willReturnCallback(static function (string $message) use (&$foundValidationLog): void {
            if (str_contains($message, 'validation_result')) {
        ...
        $response = $controller->apply($request);
        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        self::assertTrue($foundValidationLog);
    }

    public function testApplyDebugLogsWarningForEventError(): void
    {
        $dispatcher = $this->createMock(EventDispatcherInterface::class);
        $dispatcher->method('dispatch')->willReturnCallback(static function (object $event): object {
            if ($event instanceof AcroFormApplyRequestEvent) {
                $event->setError(new RuntimeException('boom'));
            }
        ...
        $logger->expects(self::once())->method('warning')->with(
            self::stringContains('AcroForm apply response: error'),
            self::anything(),
        );
    ...
    public function testApplyDebugLogsInfoForEventModifiedPdf(): void
    {
        $dispatcher = $this->createMock(EventDispatcherInterface::class);
        $dispatcher->method('dispatch')->willReturnCallback(static function (object $event): object {
            if ($event instanceof AcroFormApplyRequestEvent) {
                $event->setModifiedPdf('%PDF-1.4 modified');
            }
    ...
    public function testLoadOverridesWhenCreateTempFileThrowsSetsExtractorError(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_load_extract_ok_' . getmypid() . '.py';
        file_put_contents($script, "print('[]')\n");
        try {
            $controller = $this->createController(
            ...
            self::assertSame(Response::HTTP_OK, $response->getStatusCode());
            $data = json_decode($response->getContent(), true, 512, JSON_THROW_ON_ERROR);
            self::assertStringContainsString('Error running the fields extractor', $data['fields_extractor_error']);
        } finally {
            @unlink($script);
        }
    }

    public function testExtractFieldsWhenCreateTempFileFailsReturns500(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_extract_ok_' . getmypid() . '.py';
        file_put_contents($script, "print('[]')\n");
        try {
            $controller = $this->createController(
                fieldsExtractorScript: $script,
            ...
            self::assertSame(Response::HTTP_INTERNAL_SERVER_ERROR, $response->getStatusCode());
            $data = json_decode($response->getContent(), true, 512, JSON_THROW_ON_ERROR);
            self::assertSame('Failed to create temp file', $data['error']);
        } finally {
            @unlink($script);
        }
    }

    public function testExtractFieldsWhenWriteTempPdfFailsReturns500(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_extract_ok_' . getmypid() . '.py';
        file_put_contents($script, "print('[]')\n");
        try {
            ...
            self::assertSame(Response::HTTP_INTERNAL_SERVER_ERROR, $response->getStatusCode());
            $data = json_decode($response->getContent(), true, 512, JSON_THROW_ON_ERROR);
            self::assertSame('Failed to write temp PDF', $data['error']);
        } finally {
            @unlink($script);
        }
    }

    public function testProcessWhenCreateTempFilesFailReturns500(): void
    {
    ...
    public function testApplyWithPdfUrlUsesInjectedHttpClientAndReturnsPdf(): void
    {
        ...
        $httpClient->expects(self::once())->method('request')
            ->with('GET', 'https://allowed.example.com/doc.pdf', self::isType('array'))
            ->willReturn($response);
        ...
        self::assertSame('%PDF-1.4 modified', $result->getContent());
    }

    public function testExtractFieldsWithPdfUrlAndInjectedHttpClientReturnsFields(): void
    {
        $script = sys_get_temp_dir() . '/pdfsignable_extract_fields_ok_' . getmypid() . '.py';
        ...
        try {
            ...
            self::assertSame(Response::HTTP_OK, $response->getStatusCode());
            $data = json_decode($response->getContent(), true, 512, JSON_THROW_ON_ERROR);
            self::assertSame([['id' => 'f1']], $data['fields']);
        } finally {
            @unlink($script);
        }
    }
