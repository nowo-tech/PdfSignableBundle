...
After changing code in the bundle, refresh the demo so it uses the updated bundle: from the demo directory run `make update-bundle` (or from `demo/` run `make update-bundle-symfony8` or `make update-bundle-all`). This rebuilds the bundle assets, runs `assets:install`, and clears the cache.
