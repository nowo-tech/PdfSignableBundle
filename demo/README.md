...
Dockerized demos (Symfony 7 and Symfony 8) to try the bundle. Seventeen demo pages cover: no config, default/fixed_url/overridden config, URL as dropdown, limited boxes, same signer multiple locations, unique per name (array), page restriction, sorted boxes, no-overlap, allow-overlap, rotation (enable_rotation), defaults-by-name (box_defaults_by_name), **snap-to-grid** (snap_to_grid + snap_to_boxes), latest features (combined, including snap), predefined boxes. Touch: pinch zoom and two-finger pan work in the viewer on all demos. Demos include **Web Profiler**, **Xdebug**, and **pnpm** for building the bundle assets.
