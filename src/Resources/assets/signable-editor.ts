...
const log = createBundleLogger('pdf-signable', {
  buildTime:
...
function run(): void {
  ...
  if (!config) {
    getLogger().warn('NowoPdfSignableConfig not found, skipping init');
    return;
  }
  const { proxyUrl, strings, debug: debugMode = false } = config;

  getLogger().setDebug(debugMode);

  /** Logs to console only when debug is enabled (via config or getLogger().setDebug). */
  const debugLog = (...args: unknown[]): void => {
    getLogger().debug(...args);
  };

  /** Warns to console only when debug is enabled. */
