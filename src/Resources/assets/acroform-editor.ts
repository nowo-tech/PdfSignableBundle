...
const log = createBundleLogger('pdf-signable-acroform', {
  buildTime:
    typeof __PDF_SIGNABLE_BUILD_TIME__ !== 'undefined' ? __PDF_SIGNABLE_BUILD_TIME__ : undefined,
});
log.scriptLoaded();
setBundleLogger(log);
...
function initAcroFormEditor(root: HTMLElement): void {
  ...
  /** Removes data-np-autofill-field-type from field-name controls to match controlType behavior. */
  ...
  if (modal) {
    ...
    modal.addEventListener('change', (e) => {
      ...
      if (target instanceof HTMLSelectElement && target.id === 'acroform-edit-control-type') {
        getLogger().debug('[AcroForm edit modal] control type selected', target.value);
        updateModalVisibility(target.value);
      } else if (target instanceof HTMLSelectElement && target.id === 'acroform-edit-field-name-select') {
        getLogger().debug('[AcroForm edit modal] field name selected', target.value);
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      if (added.length) {
        getLogger().debug('[AcroForm edit modal] extension added', NP_AUTOFILL_ATTR, 'removing from', added.length, 'element(s)', added);
      }
    });
  ...
  function syncFieldNameOtherVisibility(modal: HTMLElement): void {
    ...
    getLogger().debug('[AcroForm field name other] syncFieldNameOtherVisibility', {
      modalId: modal.id,
      useFieldNameSelect,
      hasSelectInModal,
      useSelectMode,
    ...
    if (!useSelectMode) {
      getLogger().debug('[AcroForm field name other] exit: no config choice nor select in modal');
      return;
    }
    const fieldNameOtherList = modal.querySelectorAll<HTMLInputElement>('#acroform-edit-field-name-other');
    getLogger().debug('[AcroForm field name other] elements in modal', {
      hasSelect: !!fieldNameSelect,
      selectId: fieldNameSelect?.id ?? null,
      numInputsById: fieldNameOtherList.length,
      inputsByName: modal.querySelectorAll<HTMLInputElement>('input[name="acro_form_field_edit[fieldNameOther]"]').length,
    });
    if (!fieldNameSelect) {
      getLogger().debug('[AcroForm field name other] exit: #acroform-edit-field-name-select not found');
      return;
    }
    if (fieldNameOtherList.length === 0) {
      const byName = modal.querySelectorAll<HTMLInputElement>('input[name="acro_form_field_edit[fieldNameOther]"]');
      getLogger().debug('[AcroForm field name other] no input with id acroform-edit-field-name-other; by name found:', byName.length, byName.length ? Array.from(byName).map((el) => ({ id: el.id, name: el.getAttribute('name') })) : []);
    }
    ...
    fieldNameOtherList.forEach((fieldNameOther) => {
      ...
      if (group && !groupHasSelect) {
        ...
        getLogger().debug('[AcroForm edit modal] field name other group visibility', {
          display: displayVal,
      ...
      getLogger().debug('[AcroForm edit modal] field name other input visibility', {
        display: displayVal,
        id: fieldNameOther.id,
        selected: selectValue,
      });
    });
    if (fieldNameOtherList.length) {
      getLogger().debug('[AcroForm edit modal] field name other', showOther ? 'visible' : 'hidden', '(selected =', selectValue + ')');
    }
  }
  ...
  if (config.applyUrl) {
    ...
    applyBtn.addEventListener('click', () => {
      ...
      const fetchPdfThenApply = (validateOnly: boolean): Promise<ArrayBuffer | null> => {
        getLogger().debug('[AcroForm apply] request', { validateOnly, patchesCount: patches.length, fieldIds: patches.map((p) => p.fieldId) });
          ...
          .then((r) => {
            ...
            if (validateOnly && isJson) {
              return r.json().then((j: { success?: boolean; error?: string; message?: string; patches_count?: number }) => {
                getLogger().debug('[AcroForm apply] response (validate_only)', j);
                if (j.success !== true) {
            ...
            return r.blob().then((blob) => {
              ...
              buf.then((ab) => {
                getLogger().debug('[AcroForm apply] response (PDF)', {
