        ...
        $root
            ->children()
                ...
                ->arrayNode('proxy_url_allowlist')
                    ->info('When non-empty, proxy only fetches URLs matching one entry. Each entry: substring of URL, or regex if prefixed with # (e.g. #^https://example\.com/#). Empty = any public URL after SSRF checks unless proxy_url_allowlist_required is true.')
                    ->scalarPrototype()->end()
                    ->defaultValue([])
                ->end()
                ->booleanNode('proxy_url_allowlist_required')
                    ->defaultFalse()
                    ->info('When true (and proxy_enabled is true), an empty proxy_url_allowlist fails container compilation. Recommended true in production.')
                ->end()
                ...
                ->floatNode('http_timeout')
                    ->info('HTTP timeout in seconds for proxy / external PDF fetches (HttpClient). Keep below PHP max_execution_time / FrankenPHP write timeout.')
                    ->defaultValue(30.0)
                    ->min(0.1)
                ->end()
                ->floatNode('process_timeout')
                    ->info('Symfony Process timeout (and idle timeout) in seconds for AcroForm extract/apply scripts. Keep below PHP max_execution_time / FrankenPHP write timeout.')
                    ->defaultValue(60.0)
                    ->min(0.1)
                ->end()
                ->floatNode('process_script_timeout')
                    ->info('Symfony Process timeout (and idle timeout) in seconds for acroform.process_script. Keep below PHP max_execution_time / FrankenPHP write timeout.')
                    ->defaultValue(120.0)
                    ->min(0.1)
                ->end()
                ->floatNode('dependency_check_timeout')
                    ->info('Symfony Process timeout in seconds for optional pypdf probe during dependency checks.')
                    ->defaultValue(5.0)
                    ->min(0.1)
                ->end()
                ->arrayNode('signature')
